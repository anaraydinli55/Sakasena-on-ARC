...
--- END OF FILE ---
Dosyanızı incelediğimde, kod kopyalama sırasında iki adet `function AppContent() {` tanımının iç içe girdiğini ve fonksiyonların birbirini kestiğini görüyorum. Bu mükerrer (çift) kod yazımı derleyicinin hata vermesine sebep olmuştur.

Sizin için bu iki parçayı tek bir temiz **`AppContent`** çatısı altında birleştirdim ve tüm kod karmaşasını çözdüm. 

Aşağıdaki kod bloğunun tamamını kopyalayıp GitHub'daki **`App.jsx`** dosyanızın içeriğiyle değiştirerek bu derleme hatasını kalıcı olarak çözebilirsiniz:
